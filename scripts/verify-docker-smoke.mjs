...
const project = `ts-fastify-app-starter-smoke-${process.pid}`;
const workdir = await mkdtemp(join(tmpdir(), 'ts-fastify-app-starter-docker-'));
const envFile = join(workdir, '.env');
const image = `ts-fastify-app-starter:smoke-${process.pid}`;
const hostPort = 18093 + (process.pid % 1000);
const env = [
  `APP_IMAGE=${image}`,
  'APP_NAME=ts-fastify-app-starter-smoke',
  'APP_VERSION=smoke',
